...
class Ad:
    ...
    def get_title(self) -> str:
        title = self.soup.find('h1', {'class': 'brkword lheight28'})
        if title is None:
            return np.nan 
        return title.text.strip()

    def get_location(self) -> str:
        location=  self.soup.find('strong', {'class': 'c2b small'})
        if location is None:
            return np.nan
...
class OlxScraper:
    ...
    def __init__(self, section: str, range: tuple, interval: int, speed: State = State.MEDIUM) -> None:
        ...
        try:
            os.makedirs(f'data-{section}')
        except FileExistsError:
            print('data folder already exists')

        for start, end in self.ranges:
            if os.path.exists(f'data-{section}{os.sep}{self.section}_{start}_{end}.csv'):
                print(f'{self.section}_{start}_{end}.csv already exists')
                continue
            print(f'Scraping {self.section}_{start}_{end}')
            df = self.run(start, end)
            df.to_csv(
                f'data-{section}{os.sep}{self.section}_{start}_{end}.csv', index=False)
